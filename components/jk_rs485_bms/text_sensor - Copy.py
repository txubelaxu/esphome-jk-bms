...
from esphome.components import text_sensor, text_sensor_schema
...
CONFIG_SCHEMA = JK_RS485_BMS_COMPONENT_SCHEMA.extend(
    {
        cv.Optional(CONF_OPERATION_STATUS): text_sensor_schema.extend(
            {
        ...
        cv.Optional(CONF_BATTERY_TYPE): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_BATTERY_TYPE): cv.icon,
            }
        ),
        cv.Optional(CONF_PASSWORD): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_PASSWORD): cv.icon,
            }
        ),
        cv.Optional(CONF_DEVICE_TYPE): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_EMPTY): cv.icon,
            }
        ),
        cv.Optional(CONF_SOFTWARE_VERSION): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_EMPTY): cv.icon,
            }
        ),
        cv.Optional(CONF_MANUFACTURER): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_EMPTY): cv.icon,
            }
        ),
        cv.Optional(CONF_TOTAL_RUNTIME_FORMATTED): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
                cv.Optional(CONF_ICON, default=ICON_TIMELAPSE): cv.icon,
            }
        ),
        cv.Optional(CONF_NETWORK_NODES_AVAILABLE): text_sensor_schema.extend(
            {
                cv.GenerateID(): cv.declare_id(text_sensor.TextSensor),
